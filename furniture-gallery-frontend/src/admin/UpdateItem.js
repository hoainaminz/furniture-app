        ...
        const fetchData = async () => {
            try {
                ...
                setSelectedColors(colorRes.data
                    .filter(color => itemColorsRes.data.some(itemColor => itemColor.colorId === color.id)) // So sánh với colorId
                ...
                setImages(itemImagesRes.data.map(image => ({
                    preview: `http://localhost:5001/uploads/${image.imageUrl}`, // Đường dẫn đầy đủ
                    name: image.imageUrl.split('/').pop(),
                    file: null,  // Không cần file cho hình ảnh đã tồn tại
                })));
            } catch (error) {
